local cachedir  = require("gears").filesystem.get_cache_dir()
local picom     = cachedir .. "compositor.conf"
local xresources = cachedir .. "Xresources"
local xsettingsd = cachedir .. "xsettingsd"
...
local function compositor(radius, x, y, opacity)
	...
	require("awful").spawn.easy_async_with_shell(
		"pkill picom; picom --config '" .. picom .. "' &",
		function() end
	)
end
...
local function terminal(bg, fg, bl, wh, r, g, y, b, m, c)
	...
	require("awful").spawn.easy_async_with_shell(require("gears").filesystem.get_configuration_dir() .. "color/terminal.sh", function() end)
end
...
local function theme(theme, icon)
	...
	os.execute("pkill xsettingsd; xsettingsd --config '" .. xsettingsd .. "' &")
end
